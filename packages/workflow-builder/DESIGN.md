...
| `BpmnExecutionState.ts`           | Durable token, scope, invocation, gateway, Standard Loop, multi-instance, call, subscription, timer, work-item, compensation, cancellation, and exact task-resolution state foundation at state version `4`, with a version `2` executable-model fingerprint reference. Standard Loop frames cross-check activation, active/completed iteration identity, owned tokens, terminal status, and the modeled iteration cap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `BpmnExpressionRuntime.ts`        | Effect-native driver that repeatedly runs the same immutable pure-kernel operation, resolves only the first unknown exact sequence-flow or Standard Loop condition, caches it under canonical operation-local coordinates, and exposes only the final atomic batch while enforcing timeout, strict output, step limits, and Effect interruption. Standard Loop identities include activity, frame, activation, phase, and iteration so asynchronous reruns cannot alias decisions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `BpmnXml.ts`                      | Fail-closed named profile `bpmn-2.0.2-core-process-di-v3` for definitions metadata, ordinary process/subprocess control flow, source-level call-activity QNames, expression-version bindings, bounded generic-Task Standard Loop characteristics, and the current complete normalized DI slice, with canonical export and explicit mapping reports. The loop XML subset requires a version-bound `tFormalExpression` `loopCondition`, materializes `testBefore=false`, and requires a positive `loopMaximum`, although BPMN 2.0.2 permits a missing maximum and the broader optional `tExpression` condition form.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `BpmnKernel.ts`                   | Effectfully prepared, domain-separated executable fingerprints plus bounded deterministic token execution and versioned causal replay for the explicitly admitted none-event/task/subprocess/conditional/default/exclusive/parallel subset and Standard Loops on generic Tasks. The loop subset records frame and iteration identity, exact pre-test/post-test condition evidence, and maximum-reached completion; it requires a formal condition and positive `loopMaximum` and rejects combined-loop, SubProcess-loop, arbitrary-cycle, and multi-instance semantics. Fingerprinted protocol-v3 task bindings add exact success routing, one interrupting Boundary Error catch, deterministic unmapped/uncaught root failure, and resolution idempotency. Condition facts include the exact evaluator binding and bounded usage evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `BpmnExecutable.ts`               | Strict no-inference, Effectful admission from named BPMN XML profile `bpmn-2.0.2-core-process-di-v3` directly into a cryptographically prepared token kernel, including its bounded generic-Task Standard Loop intersection, while preserving diagnostics and proving import/execution/journal replay/export/re-import/recompile equivalence without lowering through the version `1` DAG. Optional immutable `taskBindings` are explicit external compile inputs passed to kernel preparation and the fingerprint; they are not sourced from BPMN XML.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
decoded from BPMN XML. State-version `4` markings and the journal-version `3`
leading header carry the version `2` fingerprint and kernel semantic version
`3`; condition events carry their exact evaluator binding and bounded usage
evidence, and `BpmnHistory` can seal the complete causal journal under an
independently anchorable history digest. Profile
`bpmn-2.0.2-core-process-di-v3` adds only the executable generic-Task Standard
Loop intersection: while-style pre-test or BPMN post-tested evaluation with a
version-bound formal condition whose `true` result means continue, plus a
mandatory positive maximum. BPMN's optional
`loopMaximum`, broader `tExpression` condition form, combined test variant,
SubProcess loops, and multi-instance constructs are not silently admitted. The

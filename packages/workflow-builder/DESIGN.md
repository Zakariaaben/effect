...
10. **Place capabilities at the narrowest justified layer.** A behavior belongs
    in the semantic core only when deterministic execution or portable meaning
    requires every backend to share it. A reusable but optional meaning belongs
    in a versioned profile/module. Organization-, vendor-, deployment-, or
    application-specific behavior belongs behind an explicit service and
    `Layer`. Placement is decided from the capability's invariants rather than
    from where one product happens to expose its UI.

Human work illustrates the rule without pre-judging its final design. A durable
work-item lifecycle may prove generic enough to be a portable semantic module,
with BPMN User Task and a future WS-HumanTask profile mapping into it.
Directories, forms/renderers, notification channels, staffing policy, and
organization-specific escalation remain injectable unless standards analysis
shows that one of their decisions changes portable workflow meaning.
...
- Schedule-to-start and start-to-close produce a distinct `AttemptTimedOut`
  ...
  Error.
- Native Activity persistence records a `Completed` receipt containing the
  complete encoded `Exit` and a backend `completedAt` observed after activity
  finalization; replay returns the same timestamp. The terminal gate compares
  this authoritative receipt with the canonical start-to-close deadline for
  success, typed error, and defect. Persisted version `2` `Succeeded` and
  `ApplicationFailed` outcomes retain their own `completedAt` for portable
  observability, but it is validated as no later than the native receipt rather
  than trusted as the arbitration clock. A terminal completion at or beyond the
  deadline becomes the timeout even if native timer delivery is late. A
  pre-deadline defect retains its non-interrupt native `Cause`; pure interruption
  publishes no terminal and interruption reasons are removed from a mixed
  terminal Cause. A wire backend may normalize defect payloads through
  `Schema.Defect`, so Cause semantics, not JavaScript object identity, are the
  portable guarantee. The outer schedule-to-close controller does not yet
  persist an equivalent absolute completion receipt for the whole retry loop,
  so post-budget defect ordering against a late-delivered schedule-to-close
  timer remains an explicit conformance gap.
...
### Native distributed node workers

`EffectWorkflowDurableQueueV3` is an opt-in transport module, not a second
workflow engine. A content-addressed route pins the artifact-selected logical
queue, worker deployment, and handler build. The workflow binds the complete
semantic operation before an `Activity` calls native `DurableQueue.process`;
the worker reloads and verifies the artifact, operation, codecs, executable
catalogue, route, deployment, and build before constructing user code.

The transport result is deliberately narrower than `NodeAttemptOutcome`: it is
only encoded success or encoded application failure. It contains no timeout and
no completion timestamp. The workflow host revalidates that result, observes
`completedAt`, and persists the final outcome through the same native Activity
contract as direct execution. Worker attestation errors cross the queue's
separate typed infrastructure channel and become Activity defects at the host;
they can never masquerade as business failures.

Backend selection is not part of the business operation document. Before
dispatch, the semantic mapper records a generic
`{ backendId, backendVersion, configurationDigest }` guard at the exact attempt
coordinates. Direct execution and optional transports supply their own pin, so
replay rejects backend or provider-configuration drift without teaching the
core about queue-specific fields. The retry controller likewise accepts an
explicit attempt executor: it still owns attempt creation, failure
classification, backoff, and schedule-to-close, while the injected function
owns transport only.

The module delegates queue storage, redelivery, waiting, worker loops, and
concurrency to native `DurableQueue`/`PersistedQueue`. Offer retry,
attestation retry, concurrency, and the native queue's non-interrupt
acquisition-failure limit are explicit application inputs. The latter is not a
business-handler retry limit because native `DurableQueue` captures handler
outcomes into its deferred result. Queue names use a complete
...
- does not claim stale-worker completion fencing, remote cancellation,
  exactly-once effects, queue-result conflict detection, or multi-machine
  failover conformance;
- keeps protocol-v1/v2 `ActivityDeliveryStore`, broker, outbox, and worker
...
Executable `PortableChildProcess/1` CallActivity frames now participate in
parent failure and operational-withdrawal closure. The exact child target
selects one policy: `CancelAndWait` retains the parent in durable `failing` or
`cancelling` state until a terminal child projection commits; `RequestCancel`
waits only for durable cancellation intent; and `Abandon` detaches without any
claim that the child stopped. Work items and human tasks, BPMN Transaction
Cancel, compensation, Terminate End Event behavior, targeted task or scope
withdrawal, and any guarantee that remote or physical work stopped remain out
of scope. This is bounded partial WCP20 Cancel Case behavior only; it does not
satisfy WCP19 Cancel Task and makes no BPMN Process Execution Conformance claim.
...
| `DigestV3.ts`                     | Descriptor-safe canonical SHA-256 envelopes with audited, domain-separated artifact, compiler-v2 plan, boundary-contract, executable-build, encoded-schema, occurrence, semantic-operation, and native-worker-route identities plus typed crypto/input failures.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `EffectWorkflowBackendV3.ts`      | Thin optional native Effect Workflow host for already-admitted protocol-v3 runs: exact artifact-versioned workflow tags, detached request/result envelopes, binding and semantic-execution provenance retaining the exact verified artifact, tenant/run idempotency and address recovery, complete child-target re-derivation against private verified provenance, handler registration, and delegated execute/poll/safe-interrupt/resume with no storage-engine SPI.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `EffectWorkflowOperationV3.ts`    | Strict bounded replay-stable naming coordinates for logical native activities, descriptor bindings, generic execution-backend guards, durable timer generations, and deferred generations. Semantic attempts use native `Activity.CurrentAttempt`; Effect Workflow remains the sole activity-result, deferred, clock, suspension, and replay implementation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `BpmnExecutionState.ts`           | Durable token, scope, invocation, gateway, Standard Loop, multi-instance, call, subscription, timer, work-item, compensation, cancellation, and exact task-resolution state foundation at state version `9`, with a version `7` executable-model fingerprint reference. Call frames retain their protocol-v3 child history and parent-close command; `failing` and `cancelling` states prevent premature parent finalization until their exact close barriers discharge. Catch state records atomic wait groups, exact Message/Timer arms, deterministic winners, loser cancellation, and one execution-global Message-delivery ledger whose dispositions include both `message-winner` and `timer-preempted`. A strict invocation-branch union separates Standard Loop iterations from Multi-Instance items. Closed Multi-Instance groups cross-check activation authority, frozen cardinality or collection source, ordered member identity and lifecycle, optional complete output aggregation, active token ownership, exact counters, completion/cancellation reason, and terminal timestamps.                                                                                                                                                                                                                                                                                                         |
| `BpmnCallActivityV3.ts`           | Portable executable `CallActivity` binding, frame, canonical `ScheduleChild` egress, target-pinned close commands, and authoritative child-event ingress/replay. It pins each expanded `calledElement` QName to one exact child target plus an explicit encoded-input expression, but owns no store, scheduler, outbox relay, child runner, or native Effect Workflow lifecycle adapter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `BpmnExpressionRuntime.ts`        | Effect-native driver that repeatedly runs the same immutable pure-kernel operation, resolves only the first unknown exact sequence-flow, Standard Loop, Multi-Instance cardinality, Multi-Instance collection, or Multi-Instance completion expression, caches it under canonical operation-local coordinates, and exposes only the final atomic batch while enforcing timeout, expected-result type, step limits, and Effect interruption. Multi-Instance coordinates include activity/group activation, the exact collection DataInput reference where applicable, and, for completion, the completed item identity plus exact counters, so asynchronous reruns cannot alias decisions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `BpmnXml.ts`                      | Fail-closed named profile `bpmn-2.0.2-core-process-di-v6` for definitions metadata, ordinary process/subprocess control flow, source-level call-activity QNames, expression-version bindings, bounded generic-Task Standard Loop characteristics, represented Multi-Instance characteristics, normal-flow Message/Timer Intermediate Catch Events, exclusive non-instantiating Event-Based Gateways, and the current normalized DI slice, with canonical export and explicit mapping reports. Multi-Instance XML round-trips cardinality, collection references, scalar input/output data items with namespace-expanded subject QNames and xsd:string names including the empty string, sequential/parallel mode, completion condition, behavior, and One/None event references in BPMN XSD order. Catch XML admits exactly one Message definition or one `timeDuration`/`timeDate` Timer definition and rejects `timeCycle`, multiple/parallel-multiple semantics, and other event definitions. Executable admission remains narrower than interchange.                                                                                                                                                                                                                                                                                                                                                    |
| `BpmnKernel.ts`                   | Effectfully prepared, domain-separated executable fingerprints plus bounded deterministic token execution and versioned causal replay for the explicitly admitted none-event/task/subprocess/conditional/default/exclusive/parallel subset, Standard Loops, `FixedMultiInstance/1`, `CollectionMultiInstance/1`, and portable `CatchEventChoice/1`. Catch waits atomically open exact Message/Timer arms. The delivery transition requires receipt `acceptedAt` to equal `services.now` while validating the exact arm and selecting against materialized Timer deadlines; sender time is not authoritative. A Timer has equality precedence, equal Timer deadlines use immutable branch ordinal, winner/continuation/loser cancellation commit together, and one global `deliveryId` ledger consumes Message winners or Timer-preempted Messages. Replay derives state only from causal journal facts without clock, expression, or race reexecution. Both Multi-Instance profiles freeze a bounded closed group once at activation, preserve stable index identities, run sequentially or in parallel with behavior All, record exact counters, fence late completion, cancel the group before Boundary Error/failure routing, and derive exact native task occurrences. Fingerprinted protocol-v3 task, data, collection, Message, authorization, and evaluator bindings retain exact execution meaning. |
...
| `BpmnExecutable.ts`               | Strict no-inference, Effectful admission from named BPMN XML profile `bpmn-2.0.2-core-process-di-v6` directly into a cryptographically prepared token kernel, including bounded generic-Task Standard Loop, Fixed Multi-Instance, Collection Multi-Instance, standalone Message/Timer catch, and Message/Timer event-choice intersections without lowering through the version `1` DAG. The CatchEventChoice XML evidence compiles and canonically round-trips the standalone shapes, and proves one equal-deadline Message/Timer gateway execution through journal replay, export, re-import, recompilation, fingerprint equivalence, and same-state replay. Optional immutable `taskBindings`, `dataDocument`, `collectionBindings`, and `messageBindings` are explicit external compile inputs passed to kernel preparation and the fingerprint; they are not inferred from BPMN XML. Unsupported Event definitions/gateway modes, progressive Multi-Instance behavior, and partial collection-output policies fail closed.                                                                                                                                                                                                                                                                                                                                                                              |
| `EffectWorkflowBpmnEventV3.ts`    | Optional native Effect Workflow backend delegation with one typed deferred per wait group, including Message-only groups, absolute idempotent `DurableClock` Timer schedules, typed first-wins Timer/state-change wake hints, and post-commit notification/recovery. It validates portable kernel authority and generates stable backend identities, but owns neither BPMN Timer state, correlation, cancellation, race selection, nor replay; first native wake is not the semantic winner. The host owns and persists the explicit `workflowName`/`executionId` address, while the Effect token is an address rather than a credential. Closed portable state can reconstruct a post-crash notification, but the adapter implements no backend SPI, persistence, inbox/outbox, or transport authentication. Memory-backed integration evidence is not production durability; persistent/cluster crash, restart, and failover conformance remains required.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
For the `EffectWorkflowRetryV3.ts` row, start-to-close arbitration uses the
native Activity `Completed.completedAt` receipt for every terminal Exit.

`EffectWorkflowBpmnCallActivityV3.ts` is the optional post-commit native child
dispatcher. It consumes strict relation-bound schedule/cancel/abandon commands,
uses Effect services for exact-target resolution and a caller-owned durable
relation authority, performs a fenced two-stage start arbitration, delegates
deterministic native start or exact-locator safe interruption, and acknowledges
abandon without touching the backend. Crash between start and locator recording
is recovered through native tenant/child-run idempotency. Every receipt
explicitly produces no semantic child event: a cooperating child host/authority
...
becomes the exact attempt timeout. This does not yet establish equivalent
absolute arbitration for a defect escaping the complete retry loop after the
outer schedule-to-close budget.
...
manifest. Optional protocol-v3 task, CallActivity, data, collection, and
Message bindings are
external compile options, passed explicitly into kernel preparation and the
fingerprint rather than decoded from BPMN XML. State-version `9` markings and
the journal-version `8` leading header carry the version `7` fingerprint and
kernel semantic version `8`; condition, CallActivity-input, collection,
Message-correlation, and
...
`bpmn-2.0.2-core-process-di-v6` admits the executable generic-Task Standard
Loop, `FixedMultiInstance/1`, `CollectionMultiInstance/1`, and
`CatchEventChoice/1` intersections.
The fixed profile evaluates a version-bound cardinality expression exactly once
at activation. The collection profile instead evaluates one exact
`loopDataInputRef` binding once, freezes its bounded canonical JSON array, and
preserves source indices, order, and duplicate values. Both profiles use stable
item identities, support sequential or parallel behavior All, and record exact
instance, active, completed, and terminated counters. A declared scalar
`inputDataItem` opts the collection profile into per-member item mapping;
without it, no implicit item is supplied. Optional complete-result aggregation
requires `loopDataOutputRef`, one scalar `outputDataItem`, a matching
collection-valued Task DataOutput declaration, and a protocol-v3 binding whose
codec validates every member output. Results are gathered by source index, not
completion order. An empty collection creates no member and produces `[]` when
aggregation is configured. Aggregation and `completionCondition` are rejected
together until an explicit partial-result policy exists.

Without collection output, an optional pinned completion condition is evaluated
after every logical completion with the exact counters. The all-join or
first-true condition emits one continuation; any already generated active
remainder is terminated, while a sequential planned suffix is never generated
or counted as terminated. Stale completion is fenced. Boundary Error and
terminal failure clean up generated members before continuing their own
semantics. The optional native Effect Workflow bridge maps group activation and
member index into distinct exact occurrences; the pure executable profiles do
not require that backend.

BPMN's optional
`loopMaximum`, broader `tExpression` condition form, combined test variant,
and SubProcess loops are not silently admitted. XML v6 can represent
Multi-Instance collection references, scalar input/output data items, and
One/None/Complex behavior metadata. The executable collection profile still
requires exact external data and evaluator bindings and rejects behavior other
than All, partial output aggregation, SubProcess/CallActivity Multi-Instance,
and open/dynamic or draining groups. Those require separate profile identities,
including `OpenForEachGroup/1` for WCP15 and WCP34/WCP36 rather than replaying
a closed journal under wider semantics. The
Effect evaluator registry performs exact full-tuple resolution with no semver
or latest fallback, while the Effect driver evaluates asynchronously by
rerunning one fixed immutable kernel input and exposes no partial batch. These
mechanisms provide portable content identity and fail-closed replay; they are not
persistent storage, a signature/MAC, a bundled FEEL/XPath implementation, or a
formal BPMN conformance claim.

`CatchEventChoice/1` is a separate portable slice, not a widening of the whole
BPMN Event lifecycle. A standalone normal-flow Intermediate Catch Event has one
Message or one Timer definition; an Event-Based Gateway is exclusive,
non-instantiating, diverging, has one incoming flow, and opens between two and
the configured maximum number of unique direct normal Message/Timer catch arms.
A compiled Message binding selects one exact ordered non-empty composite
correlation expression, payload codec, and authorization-policy build. The
atomic delivery transition requires receipt `acceptedAt` to equal
`services.now` while it validates the exact active arm, correlation, policy pin,
payload bound, and race result. Sender time and later scheduler observation are
not race authority. Exact redelivery of an already committed receipt is
recognized before this clock check and records replay audit without changing
state. Messages target only already-active arms and are not buffered by the
portable profile.

Race selection compares `acceptedAt` with all Timer deadlines. A Message strictly
before the earliest deadline wins; a Timer wins equality, the earliest eligible
Timer wins even if another Timer wakes the run, and equal deadlines use immutable
branch ordinal. One committed transition records the winner, emits its
continuation, cancels every losing arm and Timer, and records the trigger. The
execution-global `deliveryId` ledger includes both Message winners and
Timer-preempted Message attempts, preventing reuse against another catch
activation. Causal replay checks those facts and reconstructs the state without
reading a clock, invoking an evaluator, or running the race again.

The Timer elements contain `tExpression`, so BPMN's XSD does not directly
constrain their character content to `xsd:duration` or `xsd:dateTime`.
Executable admission evaluates the expression, requires a string value, and
parses that value as a bounded ISO-8601/XML Schema subset: exact non-negative
fixed day/time durations or zoned date-times normalizable to the canonical
millisecond timestamp. This is a profile rule over the evaluated value, not an
XSD validation claim.

The executable fingerprint also commits explicit budgets for Timer lexical
UTF-8 bytes, execution-state canonical bytes, transition-journal event count,
and transition-journal canonical bytes. Public parsing and kernel admission
retain hard ceilings of 4 KiB, 8 MiB, 65,536 events, and 16 MiB respectively;
a profile can only choose lower values. These bounds prevent unbounded
single-document decode and replay. They do not replace a future
checkpoint/segmentation protocol for legitimately long-running executions.

Signal, Conditional, Multiple, and Parallel Multiple Events; `timeCycle`;
Start, Boundary, throwing, Receive Task, and event-subprocess catches; and
instantiating or Parallel Event-Based Gateways are excluded. The optional
native Effect adapter schedules absolute durable clocks and publishes wake
hints only. It delegates persistence and wake-up mechanics to the selected
`WorkflowEngine`; the portable kernel remains authoritative for correlation,
selection, cancellation, the delivery ledger, and replay. This backend
delegation is not evidence of complete BPMN Process Execution Conformance.
In particular, BPMN 2.0.2 Table 10.99 requires
`MessageEventDefinition.operationRef` for an executable Process, while this
profile rejects it and supplies an external fingerprinted `MessageBinding`.
Likewise, the exact ordered correlation contract deliberately selects only the
key-based side of the alternatives described for Receive Tasks in §13.3.3.
These are explicit implementation-profile constraints and WCP16 evidence, not
normative Process Execution coverage.
...
For multi-instance execution, the comparison yields a narrow semantic split.
`FixedMultiInstance/1` snapshots one closed item set before expansion and gives
every logical item an identity independent of handler retries, deliveries, and
native Activity attempts. `CollectionMultiInstance/1` applies the same closed
identity rule to a collection snapshot: array position is semantic identity,
order and duplicates are retained, and later source mutation is irrelevant.
Its complete-success gather aggregates codec-validated outputs in input-index
order even when parallel completions arrive out of order. The semantic
cardinality and canonical-byte caps are distinct from backend
worker-concurrency and rate limits. Large groups must cross an explicit
history-segmentation or continuation boundary instead of growing one replay
journal without limit.
`OpenForEachGroup/1` will be a separate profile with durable create/close/drain
facts; it cannot be inferred from a fixed group. Effect Workflow remains the
optional owner of persistence, child execution, replay, Activities, deferreds,
clocks, and backend interruption when selected. The builder contributes
portable BPMN state, expression decisions, cancellation meaning, and exact
occurrence coordinates without cloning or requiring that runtime.
...
| P0 runtime  | Complete native lifecycle adapters and cross-process conformance: authoritative child reporting, stable operation mappings, authenticated worker builds, bounded projections, tenant isolation, backup/restore, and cutover manifests.                                 |
...
The bounded BPMN kernel now executes that `PortableChildProcess/1` slice. It
evaluates the compiled CallActivity input expression once into a bounded
protocol-v3 `EncodedPayload`, journals the evaluation, durable frame, canonical
`ChildScheduled` fact, and `ScheduleChild` outbox command, then waits for
authoritative child ingress. `applyChildEvent` validates and replays the exact
relation, child event, target contracts, and optional backend locator; it never
polls or reruns the child. Child success consumes the parent wait and routes
normal outgoing flow once. Child failure or cancellation remains a portable
terminal relation fact and is not silently promoted to BPMN Error, Cancel, or
success. The pure kernel exposes no transport or transaction boundary.
...
| 4. Effect Workflow and operations (partial)     | Thin native host, semantic operations, durable race/retry and timeout handshakes, catch wakes, and fenced CallActivity dispatch are implemented; authoritative child reporting, production APIs, telemetry, authenticated workers, and repair tools remain                                                                          | Adapter passes persistent crash/restart and multi-worker backend conformance; rolling-upgrade/replay tests; telemetry correlation and documented repair/runbooks                           |

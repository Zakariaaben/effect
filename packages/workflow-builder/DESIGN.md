...
durable reference authorities implemented; persistent adapters and complete
...
| `BpmnExecutionState.ts`           | Durable token, scope, invocation, gateway, loop, multi-instance, call, subscription, timer, work-item, compensation, and cancellation state foundation with a version `2` executable-model fingerprint reference.                                                                                                              |
...
| `BpmnKernel.ts`                   | Effectfully prepared, domain-separated executable fingerprints plus bounded deterministic token execution and versioned causal replay for the explicitly admitted none-event/task/subprocess/conditional/default/exclusive/parallel subset. Condition facts include the exact evaluator binding and bounded usage evidence.    |
...
There is currently no persistent adapter. `ExecutionStore.makeMemory` specifies
the version `1` transaction/race model, while `ExecutionAuthorityV2.makeMemory`
now runs exact protocol-v2 static-DAG starts, decisions, retries, activity
timeouts, due timers, cancellation, and schema-admitted worker completions.
`SignalAuthorityV2` specifies bounded signal admission separately. All three
authorities lose history, artifacts, receipts, leases, timers, and outbox/inbox
state on process exit, and their exact-request ledgers are not a production
retention strategy.

A persistent adapter must share the required history/outbox/inbox/timer and
cancellation-delivery transactions, define receipt expiry semantics, stream
replay or use verified discardable checkpoints, deliver loss-tolerant wakes,
and fence workers across processes. The version `1` memory reference now
retains every issued execution generation and can deliver cancellation to an
exact process-local Effect fiber. Protocol-v2 still provides only late-commit
fencing, and neither a cross-process cancellation transport nor child-workflow
authority propagation is implemented.
...
- **Transactions:** a durable decision commits history and dispatch intent in one
  backend transaction. Publishing to a separate broker is an outbox relay, not
  part of an implied distributed transaction; neither transactional state nor a
  checkpoint upgrades external I/O to exactly once.
...
- **Portable checkpoints:** authoritative recovery data is versioned semantic
  events plus encoded values or integrity-checked blob references. Process
  images, SDK continuations, provider-private rows, and warm memory are caches or
  backend details; recovery must work from empty caches through another
  conforming adapter.
...
| P0 runtime  | Persistent transaction adapters; broker acquire/visibility/ack/nack/dead-letter rules; authenticated worker-build registration, routing, drain, and retirement; bounded receipt retention; blob lifecycle; tenant isolation; backup/restore; and authority-cutover manifests. |
...
| Durable persistence               | A persistent adapter with atomic history/outbox/inbox/timer mutations, snapshots that remain discardable caches, empty-cache restore, backup/restore, corruption detection, crash injection, and failover tests.                                                                                                                                   |

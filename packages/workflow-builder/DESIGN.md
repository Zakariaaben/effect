...
| `BpmnExecutionState.ts`           | Durable token, scope, invocation, gateway, Standard Loop, multi-instance, call, subscription, timer, work-item, compensation, cancellation, and exact task-resolution state foundation at state version `6`, with a version `4` executable-model fingerprint reference. A strict invocation-branch union separates Standard Loop iterations from Multi-Instance items. Closed Multi-Instance groups cross-check activation authority, frozen cardinality or collection source, ordered member identity and lifecycle, optional complete output aggregation, active token ownership, exact counters, completion/cancellation reason, and terminal timestamps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `BpmnXml.ts`                      | Fail-closed named profile `bpmn-2.0.2-core-process-di-v5` for definitions metadata, ordinary process/subprocess control flow, source-level call-activity QNames, expression-version bindings, bounded generic-Task Standard Loop characteristics, represented Multi-Instance characteristics, and the current normalized DI slice, with canonical export and explicit mapping reports. Multi-Instance XML round-trips cardinality, collection references, scalar input/output data items with namespace-expanded subject QNames and xsd:string names including the empty string, sequential/parallel mode, completion condition, behavior, and One/None event references in BPMN XSD order. Complex behavior definitions and unrepresented data-item content remain outside the normalized slice, and executable admission is narrower than interchange.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `BpmnKernel.ts`                   | Effectfully prepared, domain-separated executable fingerprints plus bounded deterministic token execution and versioned causal replay for the explicitly admitted none-event/task/subprocess/conditional/default/exclusive/parallel subset, Standard Loops, `FixedMultiInstance/1`, and `CollectionMultiInstance/1` on generic Tasks. Both Multi-Instance profiles freeze a bounded closed group once at activation, preserve stable index identities, run sequentially or in parallel with behavior All, record exact counters, fence late completion, cancel the group before Boundary Error/failure routing, and derive exact native task occurrences. The collection profile additionally preserves source order and duplicates, exposes an item only through a declared scalar inputDataItem, and can aggregate codec-validated outputs in source-index order through a declared collection output. Output aggregation with completionCondition, progressive behavior events, SubProcess/CallActivity Multi-Instance, and open/draining groups are rejected. Fingerprinted protocol-v3 task, data, and collection bindings retain exact execution meaning.                                                                                                                                                                       |
...
| `BpmnExecutable.ts`               | Strict no-inference, Effectful admission from named BPMN XML profile `bpmn-2.0.2-core-process-di-v5` directly into a cryptographically prepared token kernel, including bounded generic-Task Standard Loop, Fixed Multi-Instance, and Collection Multi-Instance intersections, while preserving diagnostics and proving import/execution/journal replay/export/re-import/recompile equivalence without lowering through the version `1` DAG. Optional immutable `taskBindings`, `dataDocument`, and `collectionBindings` are explicit external compile inputs passed to kernel preparation and the fingerprint; they are not inferred from BPMN XML. Unsupported progressive behavior and partial collection-output policies fail closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
manifest. Optional protocol-v3 task bindings are external compile options,
passed explicitly into kernel preparation and the fingerprint rather than
decoded from BPMN XML. State-version `6` markings and the journal-version `5`
leading header carry the version `4` fingerprint and kernel semantic version
`5`; condition and collection events carry their exact evaluator binding and bounded usage
evidence, and `BpmnHistory` can seal the complete causal journal under an
independently anchorable history digest. Profile
`bpmn-2.0.2-core-process-di-v5` admits the executable generic-Task Standard
Loop, `FixedMultiInstance/1`, and `CollectionMultiInstance/1` intersections.
...
and SubProcess loops are not silently admitted. XML v5 can represent

...
| `BpmnExecutionState.ts`           | Durable token, scope, invocation, gateway, Standard Loop, multi-instance, call, subscription, timer, work-item, compensation, cancellation, and exact task-resolution state foundation at state version `5`, with a version `3` executable-model fingerprint reference. A strict invocation-branch union separates Standard Loop iterations from Multi-Instance items. Fixed Multi-Instance groups cross-check activation authority, frozen cardinality source, ordered member identity and lifecycle, active token ownership, exact counters, completion/cancellation reason, and terminal timestamps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `BpmnExpressionRuntime.ts`        | Effect-native driver that repeatedly runs the same immutable pure-kernel operation, resolves only the first unknown exact sequence-flow, Standard Loop, Multi-Instance cardinality, or Multi-Instance completion expression, caches it under canonical operation-local coordinates, and exposes only the final atomic batch while enforcing timeout, expected-result type, step limits, and Effect interruption. Multi-Instance coordinates include activity/group activation and, for completion, the completed item identity plus exact counters, so asynchronous reruns cannot alias decisions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `BpmnXml.ts`                      | Fail-closed named profile `bpmn-2.0.2-core-process-di-v4` for definitions metadata, ordinary process/subprocess control flow, source-level call-activity QNames, expression-version bindings, bounded generic-Task Standard Loop characteristics, represented Multi-Instance characteristics, and the current normalized DI slice, with canonical export and explicit mapping reports. Multi-Instance XML round-trips cardinality, collection references, sequential/parallel mode, completion condition, behavior, and One/None event references in schema order. Input/output data-item elements and complex behavior definitions remain outside the normalized slice, and executable admission is narrower than interchange.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `BpmnKernel.ts`                   | Effectfully prepared, domain-separated executable fingerprints plus bounded deterministic token execution and versioned causal replay for the explicitly admitted none-event/task/subprocess/conditional/default/exclusive/parallel subset, Standard Loops, and `FixedMultiInstance/1` on generic Tasks. The fixed Multi-Instance profile evaluates cardinality once, freezes stable items under a compiled maximum, runs sequentially or in parallel with behavior All, records exact counters, joins all or terminates the remainder on first-true completion condition, fences late completion, cancels the group before Boundary Error/failure routing, and derives exact native task occurrences. Collection mapping, progressive behavior events, output aggregation, SubProcess/CallActivity Multi-Instance, and open/draining groups are rejected. Fingerprinted protocol-v3 task bindings retain exact success/error routing and resolution idempotency.                                                                                                                                                                                                                                                                                                                                                                     |
...
| `BpmnExecutable.ts`               | Strict no-inference, Effectful admission from named BPMN XML profile `bpmn-2.0.2-core-process-di-v4` directly into a cryptographically prepared token kernel, including bounded generic-Task Standard Loop and Fixed Multi-Instance intersections, while preserving diagnostics and proving import/execution/journal replay/export/re-import/recompile equivalence without lowering through the version `1` DAG. XML collection and progressive behavior forms round-trip but executable admission fails closed. Optional immutable `taskBindings` are explicit external compile inputs passed to kernel preparation and the fingerprint; they are not sourced from BPMN XML.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
decoded from BPMN XML. State-version `5` markings and the journal-version `4`
leading header carry the version `3` fingerprint and kernel semantic version
`4`; condition events carry their exact evaluator binding and bounded usage
evidence, and `BpmnHistory` can seal the complete causal journal under an
independently anchorable history digest. Profile
`bpmn-2.0.2-core-process-di-v4` admits the executable generic-Task Standard
Loop and `FixedMultiInstance/1` intersections. The fixed Multi-Instance
intersection evaluates a version-bound cardinality expression exactly once at
activation, freezes a bounded member set with stable item identities, supports
sequential or parallel behavior All, and evaluates an optional pinned
completion condition after every logical completion with exact instance,
active, completed, and terminated counters. The all-join or first-true
condition emits one continuation; remaining members are terminated and stale
completion is fenced. Boundary Error and terminal failure clean up the group
before continuing their own semantics. The native Effect Workflow bridge maps
group activation and member index into distinct exact occurrences.

BPMN's optional
`loopMaximum`, broader `tExpression` condition form, combined test variant,
and SubProcess loops are not silently admitted. XML v4 can represent
Multi-Instance collection references and One/None/Complex behavior metadata,
but the fixed executable profile rejects collection mapping, behavior other
than All, output aggregation, SubProcess/CallActivity Multi-Instance, and
open/dynamic or draining groups. Those require separate profile identities,
including `OpenForEachGroup/1` for WCP15 and WCP34/WCP36 rather than replaying a
closed journal under wider semantics. The
...
native Activity attempts. The semantic cardinality cap is distinct from
backend worker-concurrency and rate limits. Future output collection must
aggregate in input order even when parallel completions arrive out of order,
and large groups must cross an explicit history-segmentation or continuation
boundary instead of growing one replay journal without limit.
...
owner of persistence, child execution, replay, Activities, deferreds, clocks,
and backend interruption. The builder contributes portable BPMN state,
expression decisions, cancellation meaning, and exact occurrence coordinates
without cloning that runtime.
